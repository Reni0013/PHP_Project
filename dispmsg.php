...
$sqlBanWords = "SELECT * FROM `banned_words`";

$res = '';
$result_array = array();
$runQuery = mysqli_query($conn, $sql);
$runQueryBanWords = mysqli_query($conn, $sqlBanWords);

while ($row2 = mysqli_fetch_assoc($runQueryBanWords)) {
    if (mysqli_num_rows($runQuery) > 0) {
        while ($row = mysqli_fetch_assoc($runQuery)) {

            if ($username == $row['sender_email']) {
                if ($row['message'] == $row2['words']) {
